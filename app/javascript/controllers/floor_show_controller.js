...
export default class extends Controller {
  ...
  async copyFloorInfo(event) {
    const floorName = this.titleTarget.textContent.replace('|', '').trim()
    const sequence = this.element.querySelector('[data-field="sequence"] p').textContent.trim()
    const copyText = `Floor: ${floorName}, Sequence: ${sequence}`

    try {
      await navigator.clipboard.writeText(copyText)
      this.showToast('Floor information copied to clipboard! 📋')
